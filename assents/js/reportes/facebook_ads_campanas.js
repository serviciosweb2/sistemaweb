...
$(function() {
    ...
    oTable = $('#facebook-table').dataTable({
        "bServerSide": true,
        "sAjaxSource": BASE_URL + 'reportes/listar_facebook_ads_campanas',
        "aaSorting": [[0, "desc"]],
        "sServerMethod": "POST",
        "bAutoWidth": false,
        "bPaginate": false,
    ...
    $('#facebook-table_processing').hide();
    $('#facebook-table').wrap('<div class="table-responsive"></div>');
    thead = [];
    $(aoColumnDefs).each(function() {
        thead.push(this.sTitle);
    });
    //footer
    $('#facebook-table').append('<tfoot style="background-color: #F6F6F6;">\
                                    <tr>\
                                            <td colspan="4" class="table_col_fecha_factura" style="display: table-cell;">\
                                                    <strong>'+lang.datos_facebook_ads_son_actualizados+'</strong>\
                                            </td>\
                                    </tr>\
                            </tfoot>');


    $("#facebook-table_filter").find("label").addClass("input-icon input-icon-right");
    $("#facebook-table_filter").find("label").append('<i name="icon_filters" class="icon-caret-down grey" style="margin-right: 7px; cursor: pointer"></i>');
    $("#facebook-table_filter").append($("[name=container_menu_filters_temp]").html());
    $("[name=container_menu_filters_temp]").remove();
    $("[name=div_table_filters]").hide();

    $("#facebook-table_filter").append('<i id="exportar_informe" class="icon-print grey" onclick="exportar_informe(\'pdf\');" style="cursor: pointer; margin-left: 6px;" data-original-title="" title="exportar"></i>');
    $("#facebook-table_filter").append('<i id="exportar_informe" class="icon-external-link" onclick="exportar_informe(\'csv\');" style="cursor: pointer; margin-left: 6px;" data-original-title="" title="exportar"></i>');
...
function exportar_informe(tipo_reporte){
    ...
    var sSearch = $("#facebook-table_filter").find("input[type=search]").val();
    var campana = $("[name=filtro_campana]").val();
    var fecha_desde = drp.startDate.format('YYYY-MM-DD');
    var fecha_hasta = drp.endDate.format('YYYY-MM-DD');
