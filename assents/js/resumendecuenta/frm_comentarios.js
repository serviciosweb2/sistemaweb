...
function borrarComentario(id,element){
    $.ajax({
        url: BASE_URL+"ctacte/bajaComentario",
        type: "POST",
        data:{codigo:id,ctacte:codigoCTACTE},
        dataType:"JSON",
        cache:false,
...
function addComentarios(comentarios){    
    ...
    $(comentarios).each(function(k,comentario){        
        ...
        HTML+='<div class="body">';
        HTML+='<div class="time"><i class="icon-time"></i><span class="blue">'+moment(comentario['fecha_hora'], "YYYY-MM-DD h:mm:ss").calendar()+'</span></div>';
...
function guardarComentario(element){
    $.ajax({
        url: BASE_URL+"ctacte/guardarComentario",
        type: "POSt",
