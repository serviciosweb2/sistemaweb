...
class DanfeNFePHP extends CommonNFePHP implements DocumentoNFePHP {
    ...
    */
     ...
     */
    public function montaDANFE($orientacao = '', $papel = 'A4', $logoAlign = 'C', $situacao_externa = NFEPHP_SITUACAO_EXTERNA_NONE,
            $CLASSE_PDF = false, $agregarProcom = false, $tributoIBPT = null, $porcentajeIBPTEstadual = 0){
	...
	if($CLASSE_PDF!==false){
        ...
        if (isset($this->infAdic)){
            ...
            $this->textoAdic .= "\nNão incidencia do ICMS conforme no artigo 5º, inciso VI do RICMS/MG";
        ...
        $y = $this->__dadosAdicionaisDANFE($x,$y,$hdadosadic, $agregarProcom, $tributoIBPT, $porcentajeIBPTEstadual);
...
//        return $arq;
    ...
    protected function __cabecalhoDANFE($x=0,$y=0,$pag='1',$totPag='1',$situacao_externa=NFEPHP_SITUACAO_EXTERNA_NONE){
        ...
        if ($mun == "Minas Gerais"){ // piden por excepcion de la filial cambiar estado por municipio
            $mun = "Uberlândia";
        }
    ...
    protected function __itensDANFE($x,$y, &$nInicio,$hmax,$pag=0,$totpag=0) {
        ...
        foreach ($this->det as $d) {
            if ( $i >= $nInicio) {
                ...
                $textoProduto = "MATERIAL DIDACTICO";
     ...
     * @return number Posição vertical final
     */
    protected function __dadosAdicionaisDANFE($x, $y, $h, $agregarProcom = false, $tributoIBPT  = null, $porcentajeIBPTEstadual = 0){
        //##################################################################################
        //DADOS ADICIONAIS
        ...
        $aFont = array('font'=>$this->fontePadrao,'size'=>7,'style'=>'');
        if ($agregarProcom){
            $this->textoAdic .= "\nPROCON - 151 Endereço: SCS Ed. Venâcio 2000\nBloco B-60 s/n Sala 240 - Brasília - DF CEP: 70 333 - 900";
        }
        $this->__textBox($x,$y + 1,$w-2,$h-3, $this->textoAdic   ,$aFont,'T','L',0,'',FALSE);
        if ($tributoIBPT != null){
            $valor = (float)($tributoIBPT * 4.20 / 100);
            $valor =  round($valor, 2);
            $valorEstadual = (float)($tributoIBPT * $porcentajeIBPTEstadual / 100);
            $valorEstadual = round($valorEstadual, 2);
            $tributo = "Valor Aproximado dos Tributos\n";
            $tributo .= "Nacional Federal: R$".$valor." (4,20% Fonte IBPT)\n";
            $tributo .= "Estadual: R\$$valorEstadual ($porcentajeIBPTEstadual% Fonte IBPT)\n";
            $this->__textBox(108, $y + 1, $w - 2, $h-3, $tributo, $aFont, 'T', 'L', 0, '', FALSE);
        }
        //RESERVADO AO FISCO
